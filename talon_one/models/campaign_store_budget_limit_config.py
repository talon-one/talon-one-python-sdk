...
class CampaignStoreBudgetLimitConfig(BaseModel):
    ...
    imported: StrictBool = Field(description="Indicates whether this limit configuration is managed via a CSV file.")
    __properties: ClassVar[List[str]] = ["action", "limit", "period", "entities", "imported"]
    ...
    def entities_validate_enum(cls, value):
        """Validates the enum"""
        for i in value:
            if i not in set(['Coupon', 'Referral', 'Profile', 'Identifier', 'Store', 'Session']):
                raise ValueError("each list item must be one of ('Coupon', 'Referral', 'Profile', 'Identifier', 'Store', 'Session')")
        return value
