...
class CardExpiringPointsNotificationTrigger(BaseModel):
    ...
    """ # noqa: E501
    amount: Annotated[int, Field(strict=True, ge=1)] = Field(description="The amount of period.")
    period: StrictStr = Field(description="Notification period indicated by a letter; \"w\" means week, \"d\" means day.")
    __properties: ClassVar[List[str]] = ["amount", "period"]

    @field_validator('period')
    def period_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['w', 'd']):
            raise ValueError("must be one of enum values ('w', 'd')")
        return value
