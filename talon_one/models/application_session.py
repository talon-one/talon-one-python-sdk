...
class ApplicationSession(BaseModel):
    ...
    profileintegrationid: Optional[Annotated[str, Field(strict=True, max_length=1000)]] = Field(default=None, description="Integration ID of the customer for the session.", json_schema_extra={"examples": ["382370BKDB946"]})
    coupon: StrictStr = Field(description="Any coupon code entered.", json_schema_extra={"examples": ["BKDB946"]})
    referral: StrictStr = Field(description="Any referral code entered.", json_schema_extra={"examples": ["BKDB946"]})
    state: StrictStr = Field(description="Indicates the current state of the session. Sessions can be created as `open` or `closed`. The state transitions are:  1. `open` → `closed` 2. `open` → `cancelled` 3. `closed` → `cancelled` or `partially_returned` 4. `partially_returned` → `cancelled`  For more information, see [Customer session states](https://docs.talon.one/docs/dev/concepts/entities/customer-sessions). ", json_schema_extra={"examples": ["closed"]})
