...
class CampaignTemplate(BaseModel):
    ...
    name: Annotated[str, Field(min_length=1, strict=True)] = Field(description="The campaign template name.", json_schema_extra={"examples": ["Discount campaign"]})
    description: StrictStr = Field(description="Customer-facing text that explains the objective of the template.", json_schema_extra={"examples": ["This is a template for a discount campaign."]})
    instructions: StrictStr = Field(description="Customer-facing text that explains how to use the template. For example, you can use this property to explain the available attributes of this template, and how they can be modified when a user uses this template to create a new campaign.", json_schema_extra={"examples": ["Use this template for discount campaigns. Set the campaign properties according to the campaign goals, and don't forget to set an end date."]})
    ...
    active_ruleset_id: Optional[StrictInt] = Field(default=None, description="The ID of the ruleset this campaign template will use.", alias="activeRulesetId", json_schema_extra={"examples": [5]})
    tags: Optional[Annotated[List[Annotated[str, Field(min_length=1, strict=True, max_length=50)]], Field(max_length=50)]] = Field(default=None, description="A list of tags for the campaign template.", json_schema_extra={"examples": [["discount"]]})
    reevaluate_on_return: StrictBool = Field(description="Indicates whether campaigns created from this template should be reevaluated when a customer returns an item.", alias="reevaluateOnReturn", json_schema_extra={"examples": [True]})
    ...
    applications_ids: List[StrictInt] = Field(alias="applicationsIds", json_schema_extra={"examples": [[1, 2, 3]]})
    ...
    campaign_type: Optional[StrictStr] = Field(default='advanced', description="The campaign type. Possible type values:   - `cartItem`: Type of campaign that can apply effects only to cart items.   - `advanced`: Type of campaign that can apply effects to customer sessions and cart items. ", alias="campaignType", json_schema_extra={"examples": ["advanced"]})
    campaigns_count: Optional[StrictInt] = Field(default=None, description="The number of Campaigns created from this template.", alias="campaignsCount", json_schema_extra={"examples": [3]})
    updated: Optional[datetime] = Field(default=None, description="Timestamp of the most recent update to the campaign template or any of its elements.", json_schema_extra={"examples": ["2022-08-24T14:15:22Z"]})
    updated_by: Optional[StrictStr] = Field(default=None, description="Name of the user who last updated this campaign template, if available.", alias="updatedBy", json_schema_extra={"examples": ["Jane Doe"]})
    valid_application_ids: List[StrictInt] = Field(description="The IDs of the Applications that are related to this entity.", alias="validApplicationIds", json_schema_extra={"examples": [[1, 2, 3]]})
    is_user_favorite: Optional[StrictBool] = Field(default=False, description="A flag indicating whether the user marked the template as a favorite.", alias="isUserFavorite", json_schema_extra={"examples": [False]})
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "validApplicationIds": obj.get("validApplicationIds"),
            "isUserFavorite": obj.get("isUserFavorite") if obj.get("isUserFavorite") is not None else False
        })
        return _obj
