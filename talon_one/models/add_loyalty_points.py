...
class AddLoyaltyPoints(BaseModel):
    ...
    pending_duration: Optional[StrictStr] = Field(default=None, description="The amount of time before the points are considered valid.  The time format is either: - `immediate` or, - `on_action` or, - an **integer** followed by one letter indicating the time unit.  Examples: `immediate`, `30s`, `40m`, `1h`, `5D`, `7W`, `10M`, `15Y`, `on_action`.  Available units:  - `s`: seconds - `m`: minutes - `h`: hours - `D`: days - `W`: weeks - `M`: months - `Y`: years  You can round certain units up or down: - `_D` for rounding down days only. Signifies the start of the day. - `_U` for rounding up days, weeks, months and years. Signifies the end of the day, week, month or year. ", alias="pendingDuration")
    pending_until: Optional[datetime] = Field(default=None, description="Date and time after the points are considered valid. The value should be provided in RFC 3339 format. If passed, `pendingDuration` should be omitted. ", alias="pendingUntil")
    subledger_id: Optional[StrictStr] = Field(default=None, description="ID of the subledger the points are added to. If there is no existing subledger with this ID, the subledger is created automatically.", alias="subledgerId")
    application_id: Optional[StrictInt] = Field(default=None, description="ID of the Application that is connected to the loyalty program. It is displayed in your Talon.One deployment URL.", alias="applicationId")
