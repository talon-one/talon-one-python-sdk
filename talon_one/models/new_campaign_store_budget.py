...
class NewCampaignStoreBudget(BaseModel):
    ...
    """ # noqa: E501
    action: StrictStr
    store_limits: List[NewCampaignStoreBudgetStoreLimit] = Field(description="The set of budget limits for stores linked to the campaign.", alias="storeLimits")
    period: Optional[StrictStr] = None
    __properties: ClassVar[List[str]] = ["action", "storeLimits", "period"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['setDiscount']):
            raise ValueError("must be one of enum values ('setDiscount')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in store_limits (list)
        _items = []
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "storeLimits": [NewCampaignStoreBudgetStoreLimit.from_dict(_item) for _item in obj["storeLimits"]] if obj.get("storeLimits") is not None else None,
