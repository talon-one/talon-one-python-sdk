# coding: utf-8

"""
    Talon.One API

    Use the Talon.One API to integrate with your application and to manage applications and campaigns:  - Use the operations in the [Integration API section](#integration-api) are used to integrate with our platform - Use the operation in the [Management API section](#management-api) to manage applications and campaigns.  ## Determining the base URL of the endpoints  The API is available at the same hostname as your Campaign Manager deployment. For example, if you access the Campaign Manager at `https://yourbaseurl.talon.one/`, the URL for the [updateCustomerSessionV2](https://docs.talon.one/integration-api#operation/updateCustomerSessionV2) endpoint is `https://yourbaseurl.talon.one/v2/customer_sessions/{Id}` 

    The version of the OpenAPI document: 
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from talon_one.models.customer_profile_integration_request_v2 import CustomerProfileIntegrationRequestV2

class TestCustomerProfileIntegrationRequestV2(unittest.TestCase):
    """CustomerProfileIntegrationRequestV2 unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> CustomerProfileIntegrationRequestV2:
        """Test CustomerProfileIntegrationRequestV2
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `CustomerProfileIntegrationRequestV2`
        """
        model = CustomerProfileIntegrationRequestV2()
        if include_optional:
            return CustomerProfileIntegrationRequestV2(
                attributes = {Language=english, ShippingCountry=DE},
                evaluable_campaign_ids = [10, 12],
                audiences_changes = talon_one.models.profile_audiences_changes.ProfileAudiencesChanges(
                    adds = [2, 4], 
                    deletes = [7], ),
                response_content = [triggeredCampaigns, customerProfile]
            )
        else:
            return CustomerProfileIntegrationRequestV2(
        )
        """

    def testCustomerProfileIntegrationRequestV2(self):
        """Test CustomerProfileIntegrationRequestV2"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
